...
//#endregion
//#region src/index.ts
const HUMAN_SPEED = 200;
const ZOMBIE_SPEED = 140;
const INFECTION_RADIUS = 40;
const TICK_MICROS = 50000n;
const ROUND_RESET_DELAY_MICROS = 10000000n;
const BASE_MAP_SIZE = 2e3;
const MIN_PLAYERS_TO_START = 1;
const TICK_DT_SEC = Number(TICK_MICROS) / 1e6;
function mapSizeForPlayers(count) {
	if (count <= 0) return BASE_MAP_SIZE;
	const size = Math.sqrt(count) * 400;
	return Math.max(BASE_MAP_SIZE, Math.min(size, 8e3));
}
...
const DOOR_GAP = 40;
const MARGIN = 120;
function collidesWithObstacle(obstacles, px, py, halfSize) {
	for (const o of obstacles) {
		const obsLeft = o.x - o.width / 2;
		const obsRight = o.x + o.width / 2;
		const obsTop = o.y - o.height / 2;
		const obsBottom = o.y + o.height / 2;
		if (px - halfSize < obsRight && px + halfSize > obsLeft && py - halfSize < obsBottom && py + halfSize > obsTop) return true;
	}
	return false;
}
function insertObstacle(ctx, groupId, x, y, width, height, obstacleType) {
...
function generateBuilding(ctx, roundNum, index, mapW, mapH) {
	...
	const gap = DOOR_GAP;
	const groupId = BigInt(1e3 + index);
	...
	insertObstacle(ctx, groupId, cx + halfW + t / 2, cy - halfH + leftTopSegH / 2, t, leftTopSegH, "building_wall");
	insertObstacle(ctx, groupId, cx + halfW + t / 2, cy + halfH - leftBottomSegH / 2, t, leftBottomSegH, "building_wall");
	const divCount = deterministicHash(seedBase + "div") < .5 ? 1 : 2;
	for (let d = 0; d < divCount; d++) {
		const vert = deterministicHash(seedBase + `div-${d}`) < .5;
		const gapPos = deterministicHash(seedBase + `divgap-${d}`);
		if (vert) {
			const divX = cx - halfW + w * (.3 + .4 * d) / divCount;
			const segH = halfH * gapPos * .8;
			insertObstacle(ctx, groupId, divX, cy - halfH + segH / 2, t, segH, "building_wall");
			insertObstacle(ctx, groupId, divX, cy + halfH - segH / 2, t, segH, "building_wall");
		} else {
			const divY = cy - halfH + h * (.35 + .3 * d) / divCount;
			const segW = halfW * gapPos * .8;
...
function generateRuin(ctx, roundNum, index, mapW, mapH) {
	...
	const groupId = BigInt(2e3 + index);
	...
	for (let i = 0; i < numFrags && pos < len; i++) {
		...
		const perp = (deterministicHash(seedBase + `p-${i}`) - .5) * 24;
		insertObstacle(ctx, groupId, baseX + (isHoriz ? pos + segLen / 2 : perp), baseY + (isHoriz ? perp : pos + segLen / 2), isHoriz ? segLen : segW, isHoriz ? segW : segLen, "ruin");
		pos += segLen;
	}
}
function generateTrees(ctx, roundNum, startIndex, count, mapW, mapH) {
...
function generateAllObstacles(ctx, roundNum, mapW, mapH) {
	for (const o of ctx.db.Obstacle.iter()) ctx.db.Obstacle.id.delete(o.id);
...
const tick = spacetimedb.reducer((ctx) => {
	...
	if (!config.roundActive) {
		...
		if (players.length >= MIN_PLAYERS_TO_START) {
			...
			for (const p of players) ctx.db.Player.identity.update({
				...p,
				score: 0n
			});
			ctx.db.GameConfig.id.update({
	...
	const infectedThisTick = /* @__PURE__ */ new Set();
	...
	if (allPlayers.filter((p) => !p.isZombie).length === 0 && allPlayers.length >= 2) ctx.db.GameConfig.id.update({
		...cfg,
		roundActive: false,
		roundEndMicros: now
	});
});
const set_input = spacetimedb.reducer({
	dirX: t.f64(),
	dirY: t.f64()
}, (ctx, { dirX, dirY }) => {
...
const set_name = spacetimedb.reducer({ name: t.string() }, (ctx, { name }) => {
	...
	const finalName = (name ?? "").trim().slice(0, 32) || "Player";
	for (const other of ctx.db.Player.iter()) if (other.identity.toHexString() !== ctx.sender.toHexString() && other.name.toLowerCase() === finalName.toLowerCase()) throw new SenderError("Name already taken");
	ctx.db.Player.identity.update({
		...player,
		name: finalName
	});
...
const init = spacetimedb.init((ctx) => {
	if (!ctx.db.GameConfig.id.find(0n)) ctx.db.GameConfig.insert({
		id: 0n,
		roundActive: false,
		roundNumber: 0n,
		roundStartMicros: 0n,
...
const onConnect = spacetimedb.clientConnected((ctx) => {
	...
	const isFirst = [...ctx.db.Player.iter()].length === 0;
	...
	if (config) ctx.db.GameConfig.id.update({
		...config,
		mapWidth: newSize,
		mapHeight: newSize
	});
...
const onDisconnect = spacetimedb.clientDisconnected((ctx) => {
	...
	if (config && zombies.length === 0 && players.length > 0) {
		const newZombie = players[Number(config.roundNumber % BigInt(players.length))];
		ctx.db.Player.identity.update({
			...newZombie,
			isZombie: true
		});
...
//#endregion
export { spacetimedb as default, init, onConnect, onDisconnect, set_input, set_name, tick };
//# debugId=c7791563-c34e-438e-80a9-4065b8a69a4c
//# sourceMappingURL=data:application/json;charset=utf-8;base64,