...
const tick = spacetimedb.reducer((ctx) => {
	...
	if (!config.roundActive) {
		...
		if ([...ctx.db.Player.iter()].length >= MIN_PLAYERS_TO_START) ctx.db.GameConfig.id.update({
			...config,
			roundActive: true,
			roundNumber: config.roundNumber + 1n,
			roundStartMicros: now,
